...
/* Query #1 One Table and Multiple Conditions in WHERE Clause */
SELECT * FROM BOOKS
...
/* Query #2 Two Related Tables and Multiple Conditions in WHERE Clause */
...
/* Query #3 Grouping with GROUP BY */
SELECT STDPOINT, COUNT(*) AS COUNT
FROM STUDENTS
GROUP BY STDPOINT;


/* Query #4 Two Related Tables and GROUP BY Grouping */
...
/* Query #5 Grouping and HAVING Clause (Conditions for Groups) */
SELECT P.PUBLISHERFULLNAME AS PUBLISHER_NAME, COUNT(B.ISBN) AS BOOK_COUNT
FROM PUBLISHERS P
INNER JOIN BOOKS B ON P.PUBLISHERID = B.PUBLISHERID
GROUP BY P.PUBLISHERFULLNAME
...
/* Query #6 Two Related Tables, Grouping and HAVING Clause (Conditions for Groups) */
SELECT S.STDFIRSTNAME AS STUDENT_NAME, COUNT(BS.ISBN) AS BOOK_COUNT
FROM STUDENTS S
INNER JOIN BOOKS_STUDENTS BS ON S.STDID = BS.STDID
GROUP BY S.STDFIRSTNAME
HAVING COUNT(BS.ISBN) >= 1;


/* Query #7 Subquery in SELECT Clause */
SELECT
    C.CATEGORYNAME AS CATEGORY_NAME,
    (SELECT COUNT(*) FROM BOOKS_CATEGORIES BC WHERE BC.CATEGORYID = C.CATEGORYID) AS BOOK_COUNT
FROM
    CATEGORIES C;


/* Query #8 Subquery in FROM Clause */
SELECT *
FROM (
    SELECT *
    FROM BOOKS
...
/* Query #10 Subquery in HAVING Clause */
...
/* Query #11 UNION, INTERSECT, MINUS Constructions */
...
/* Query #14 RIGHT JOIN and LEFT JOIN */
SELECT
